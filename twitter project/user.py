...
class User:
    ...
    def twitter_request(self, url, verb = 'GET'):
        authoried_token = oauth2.Token(self.oauth_token,self.oauth_token_secret)
        authoried_client = oauth2.Client(consumer,authoried_token)

        response, content = authoried_client.request(url,verb)
        if response.status != 200:
            print('An Error occured during searching')
